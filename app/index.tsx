...
export default function HomeScreen() {
  ...
  const handlePress = (idx: number) => {
    ...
    newButtons[idx] =
      newButtons[idx] === Color.White ? Color.Black : Color.White;
   ...
   * @returns
   */
  const buttonsToChar = (buttonArr: Color[]): string => {
    let binaryDots: string = buttonArr
      .map((n) => Object.values(Color).indexOf(n))
      .join("");
  ...
  return (
    <SafeAreaProvider>
      <SafeAreaView
        style={{ flexDirection: "column", alignItems: "center", top: 50 }}
      >
        {createRow(0)}
        {createRow(1)}
        {createRow(2)}
